...
class PatternLockView @JvmOverloads constructor(
    ...
    init {
        context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.PatternLockViewCustom,
            0,
            0
        ).apply {
            try {
                attrIsDotAnimate =
                    getBoolean(R.styleable.PatternLockViewCustom_patternLock_isAnimate, true)
                attrDotColor =
                    getColor(R.styleable.PatternLockViewCustom_patternLock_dotColor, Color.DKGRAY)
                attrLineColor =
                    getColor(R.styleable.PatternLockViewCustom_patternLock_lineColor, Color.DKGRAY)
                attrSuccessDotColor =
                    getColor(
                        R.styleable.PatternLockViewCustom_patternLock_successDotColor,
                        Color.BLUE
                    )
                attrSuccessLineColor =
                    getColor(
                        R.styleable.PatternLockViewCustom_patternLock_successLineColor,
                        Color.BLUE
                    )
                attrErrorDotColor =
                    getColor(R.styleable.PatternLockViewCustom_patternLock_errorDotColor, Color.RED)
                attrErrorLineColor =
                    getColor(
                        R.styleable.PatternLockViewCustom_patternLock_errorLineColor,
                        Color.RED
                    )
            } finally {
                recycle()
        ...
        event?.let { motionEvent ->
            ...
            when (motionEvent.action) {
                ...
                MotionEvent.ACTION_UP -> {
                    if (markedDotList.isNotEmpty() && markedDotList.size >= minCount) {
                        when (stageState) {
                        ...
                        updateViewState(state)
                        onChangeStateListener?.invoke(state)
                    } else if (markedDotList.isNotEmpty()) {
...
//                    setDotViewColor(attrDotColor)
    ...
    private fun addInitialData() {
        if (initialDotList.isNotEmpty()) return
